...
interface HomeContentDto {
  readonly hero: {
    readonly heading: readonly HeadingSegment[] | null;
    readonly subheading: string | null;
    readonly imageUrl: string | null;
    readonly stats: readonly HeroStat[] | null;
  } | null;
}
...
export class HomeContentService {
  ...
  private merge(dto: HomeContentDto | null): HomeContent {
    const hero = dto?.hero;
    // Drop stats the CMS hasn't filled with lines (e.g. left over from an older shape).
    const validStats = (hero?.stats ?? []).filter((stat) => stat.lines?.length);

    return {
      ...HOME_CONTENT_PLACEHOLDER,
      hero: {
        // Empty arrays mean "not set in the CMS" → keep the placeholder.
        heading: hero?.heading?.length ? hero.heading : HOME_CONTENT_PLACEHOLDER.hero.heading,
        ...
        stats: validStats.length ? validStats : HOME_CONTENT_PLACEHOLDER.hero.stats,
      },
    };
  }
}
